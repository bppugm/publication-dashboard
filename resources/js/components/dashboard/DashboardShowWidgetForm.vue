...
export default {
  ...
  computed: {
    ...
    chartData() {
      var labelArr = this.chartForm.map(item => item.label)
      var datasetArr = this.chartForm.map(item => item.dataset)
      var colorArr = this.chartForm.map(item => item.color)
      return {
        labels: labelArr,
        datasets: [{
          data: datasetArr,
  ...
  methods: {
    ...
    addChartData() {
      this.chartForm.push({ ...this.chartFormDefault });
    },
    removeChartData(index) {
      this.chartForm.splice(index, 1);
    },
    ...
    submit() {
      this.$emit("submitted", {
        widget: this.form.type == "chart" ?
          { ...this.form, chartOptions: this.chartOptions } : 
          this.form,
